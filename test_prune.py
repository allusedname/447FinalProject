...
class opt():
    ...
    model = 'checkpoints/yolov3_ckpt_99.pth'
...
# Avoid pruning the highest threshold in all channels(The maxiumum min of each BN layer's gamma would be the threshold)
highest_thre = []
for idx in prune_idx:
    highest_thre.append(model.module_list[idx][1].weight.data.abs().max().item())
highest_thre = min(highest_thre)

# Find the percentage corresponding to the highest_thre
...
#%%
# Test the performance of the model and get the parameter's number.
compact_model_metric = eval_model(compact_model)

#%%
# Compare the number of parameters and performance score after pruning. 
metric_table = [
    ["Metric", "Before", "After"],
...
#%%
# Generate cfg file after pruning and save the model.
